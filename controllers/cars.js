...
const PATHCSV = __dirname+'/../csv/cars.csv';
const PATHSTYLES = __dirname+'/../data/carStyle.json';
...
function createCsv(req, res) {
	...
	// Save the csv data into the cars.csv file
	var result = CSV.create(PATHCSV, data);
...
function createStyles(req, res) {
	...
	req.on('end', () => {
		...
		// save the data
		fs.writeFile(PATHSTYLES, JSON.stringify(data), err => {
			if(err) return _errorResponse(res, 'Could not save new car.');
			return _jsonResponse(res, data);
		});
